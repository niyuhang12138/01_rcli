//! Text sign/verify and key generate
...
pub fn process_text_sign(
    ...
    let signer: Box<dyn TextSigner> = match format {
        TextSignFormat::Black3 => Box::new(Blake3::try_new(key)?),
...
pub fn process_text_key_generate(format: TextSignFormat) -> Result<HashMap<&'static str, Vec<u8>>> {
    match format {
        TextSignFormat::Black3 => Blake3::generate(),
...
mod tests {
    ...
    fn test_process_text_verify() -> Result<()> {
        let mut reader = "hello!".as_bytes();
        let format = TextSignFormat::Black3;
        let sig = "MuFGGlzfvnjfSbGNAxK8Jqsnr_4P9Xz6ircoL2H_41Q";
